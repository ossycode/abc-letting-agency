...
export default function UserDropdown() {
  ...
  const displayName = useMemo(() => {
    ...
    if (!isAuthenticated || !me) return "Guest";
  ...
  const initials = useMemo(() => {
    ...
    if (!parts.length && me.email) return me.email[0]?.toUpperCase() ?? "U";
    return (
      parts
        .map((p) => p?.[0] ?? "")
        .join("")
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-white">
            {initials}
          </span>
        </span>
